...
# 1. 页面配置与样式
# ==============================================================================
st.set_page_config(page_title="Quant Sniper Pro (Bi-Directional)", layout="wide", page_icon="⚡")
...
def get_support_trendline(df, lookback=1000):
    ...
    for idx_A in potential_start_points:
        ...
        for idx_B in trough_indexes:
            ...
            if price_B <= price_A: continue # 支撑线通常是上升的
...
def generate_option_plan(ticker, current_price, signal_type, rsi):
    ...
    if "突破" in signal_type or "双重共振" in signal_type:
        plan['name'] = "🚀 做多 (Call)"
...
def plot_chart(df, res, height=600):
    ...
    if res['trend_sup']:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines', name='Support', line=dict(color='#FF00FF', width=2) # Magenta
        ))
    ...
    if res['abc']:
        ...
        for ratio in [1.0, 1.618, 2.618]:
            ...
            fig.add_shape(type="line", x0=pC['date'], y0=lvl, x1=df.index[-1]+timedelta(days=10), y1=lvl, line=dict(color="gray", width=1, dash="dot"))
            fig.add_annotation(x=df.index[-1], y=lvl, text=f"{ratio}: ${lvl:.2f}", showarrow=False, xanchor="left", font=dict(color="gray", size=10))

    # 6. 默认缩放
    default_start_date = df.index[-1] - timedelta(days=90)
    fig.update_layout(
        template="plotly_dark", height=height, margin=dict(l=0,r=80,t=30,b=0),
...
def analyze_ticker_pro(ticker, interval="1d", lookback="5y", threshold=0.06):
    try:
        ...
        trend_sup = get_support_trendline(df, lookback=lb_trend)    # 支撑线 (新增)
        ...
        # 看空信号 (优先级高于普通WAIT，低于双重共振)
        is_breakdown = trend_sup and trend_sup['breakdown']
        if is_breakdown:
            # 只有当不是特别强的多头信号时，才显示空头
            if "双重" not in signal:
                signal = "📉 趋势线跌破"
                signal_color = "#FF00FF" # Magenta
                reasons.append("跌破长期上升支撑")

        # 止损位：如果是做多，用ATR下方；如果是做空，用ATR上方
        stop_loss_level = current_price - (2.0 * current_atr) if "跌破" not in signal else current_price + (2.0 * current_atr)
...
if mode == "🔍 单股狙击 (Live)":
    st.title("🛡️ 狗蛋风控指挥舱 (Bi-Directional)")
