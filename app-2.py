...
def plot_chart(df, res, height=600):
    ...
    if res['abc']:
        ...
        target_100 = pC['price'] + height_AB * 1.0
        target_1618 = pC['price'] + height_AB * 1.618
        target_2618 = pC['price'] + height_AB * 2.618
        ...
        # (B) 绘制 Fib 线
        # 1.0 Level
        fig.add_shape(type="line", x0=start_date, y0=target_100, x1=last_date, y1=target_100,
                      line=dict(color="gray", width=1, dash="dot"))
        fig.add_annotation(x=last_date, y=target_100, text="Fib 1.0", showarrow=False, xanchor="left", font=dict(color="gray"))

        # 1.618 Level (重点)
        fig.add_shape(type="line", x0=start_date, y0=target_1618, x1=last_date, y1=target_1618,
                      line=dict(color="#00FF00", width=2))
        fig.add_annotation(x=last_date, y=target_1618, text="🎯 Fib 1.618 (TP1)", showarrow=False, xanchor="left", font=dict(color="#00FF00", size=14, weight="bold"))

        # 2.618 Level
        fig.add_shape(type="line", x0=start_date, y0=target_2618, x1=last_date, y1=target_2618,
                      line=dict(color="gold", width=1, dash="dot"))
        fig.add_annotation(x=last_date, y=target_2618, text="🚀 Fib 2.618 (TP2)", showarrow=False, xanchor="left", font=dict(color="gold"))
    ...
    fig.update_layout(
        ...
        height=height, 
        margin=dict(l=0,r=60,t=30,b=0), # 右侧留给 Fib 标签
